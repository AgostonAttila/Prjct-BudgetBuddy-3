    ...
    ILogger<ExcelImportService> logger) : IDataImportService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            throw new InvalidOperationException("Import failed. All transactions have been rolled back.", ex);
        }
        finally
        {
        ...
        Dictionary<string, Guid> categories)
    {
        ...
        {
            throw new InvalidOperationException($"Account '{accountName}' not found. Please create account first.");
        }
        ...
        {
            throw new InvalidOperationException($"Invalid date format: '{dateStr}'. Expected ISO 8601 format (e.g., 2022-12-31T00:00:00.000Z).");
        }
        var localDate = LocalDate.FromDateTime(dateTime);

        // Map Hungarian text to enums
    ...
    private static TransactionType MapTransactionType(string hungarianText)
    {
        return hungarianText.ToLower() switch
        {
            ...
            _ => throw new InvalidOperationException($"Unknown transaction type: {hungarianText}")
        };
    }
    ...
    private static decimal ParseCellDecimal(IXLCell cell)
    {
        ...
        return decimal.TryParse(
            ...
            out var parsed)
            ? parsed
            : throw new InvalidOperationException($"Invalid numeric value: '{cell.GetString()}'");
    }
}

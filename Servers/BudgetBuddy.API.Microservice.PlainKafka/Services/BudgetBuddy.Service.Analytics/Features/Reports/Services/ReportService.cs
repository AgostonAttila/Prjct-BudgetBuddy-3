    ...
    ILogger<ReportService> logger) : CurrencyServiceBase(currencyConversionService, logger), IReportService
{
            ...
            CancellationToken cancellationToken = default)
    {
        ...
        var categoryTotals = groupedData
            ...
            .Select(catGroup => (
                ...
                CategoryName: catGroup.Key.HasValue && categoryNames.TryGetValue(catGroup.Key.Value, out var info)
                    ? info.Name
                    : "Uncategorized",
    ...
    private sealed record MonthlySummaryRawData(
        TransactionType TransactionType,
        string CurrencyCode,
        decimal TotalAmount,
        int Count);

    private sealed record CategoryRawData(
        string CategoryName,
        string CurrencyCode,
        decimal Amount,
        int Count);

    private sealed record DailyRawData(
        int Day,
        TransactionType TransactionType,
        string CurrencyCode,
        decimal Amount,
            ...
            CancellationToken cancellationToken)
    {
        ...
        // Top categories: resolve real category names from the local CategorySnapshot read model.
        var categoryExpenseRows = await transactionQueryService
            .GetExpensesGroupedByCategoryAsync(userId, startDate, endDate, cancellationToken);

        var categoryIds = categoryExpenseRows
            .Where(r => r.CategoryId.HasValue)
            .Select(r => r.CategoryId!.Value)
            .Distinct();
        var categoryNames = await categoryQueryService.GetCategoriesByIdsAsync(categoryIds, cancellationToken);

        var topCategoriesData = categoryExpenseRows
            .Select(r => new CategoryRawData(
                r.CategoryId.HasValue && categoryNames.TryGetValue(r.CategoryId.Value, out var info)
                    ? info.Name
                    : "Uncategorized",
                r.CurrencyCode, r.Amount, r.Count))
            .ToList();
    ...
    private static List<CategorySummary> ProcessTopCategories(
        List<CategoryRawData> topCategoriesData,
        Dictionary<string, decimal> exchangeRates)
    {
    ...
    private static List<DailySummary> ProcessDailyBreakdown(
        List<DailyRawData> dailyData,
        Dictionary<string, decimal> exchangeRates)
    {
        return dailyData

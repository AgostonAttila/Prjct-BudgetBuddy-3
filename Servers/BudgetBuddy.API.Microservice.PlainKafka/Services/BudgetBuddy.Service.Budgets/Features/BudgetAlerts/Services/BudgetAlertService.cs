...
{
    private readonly decimal _warningThreshold  = configuration.GetValue<decimal>("BudgetAlerts:WarningThresholdPercent",  80m);
    private readonly decimal _exceededThreshold = configuration.GetValue<decimal>("BudgetAlerts:ExceededThresholdPercent", 100m);
    ...
    {
        if (utilizationPercentage >= _exceededThreshold)
        {
            return AlertLevel.Exceeded;
        }

        if (utilizationPercentage >= _warningThreshold)
        {
        ...
        decimal remaining)
    {
        ...
        {
            AlertLevel.Warning =>
                $"?? You've used {utilizationPercentage}% of your budget. {Math.Abs(remaining):F2} remaining.",
            AlertLevel.Exceeded =>

...
public static class SecurityExtensions
{
    public static WebApplicationBuilder AddSecurity(this WebApplicationBuilder builder, IConfiguration configuration)
    {
        ...
        builder.Services.AddOptions<RateLimitConfig>()
        ...
        // Dev/Staging: /tmp/dp-keys — writable by the app user in Docker containers
        ...
        {
            dataProtection.PersistKeysToFileSystem(new DirectoryInfo("/tmp/dp-keys"));
        }
    ...
    private static void AddRateLimiting(this IServiceCollection services)
    {
        ...
        services.AddOptions<RateLimiterOptions>()
            .Configure<IOptions<RateLimitConfig>>((options, rateLimitOptions) =>
            {
                ...
                // S-07: Per-user (sub claim) fixed window — falls back to IP for anonymous requests.
                // Prevents a single authenticated user from monopolising shared rate limit headroom.
                options.AddPolicy("perUser", httpContext =>
                {
                    ...
                    return RateLimitPartition.GetFixedWindowLimiter(key,
                        _ => new FixedWindowRateLimiterOptions
                        {
                            PermitLimit          = config.PerUser.PermitLimit,
                            Window               = TimeSpan.FromMinutes(config.PerUser.WindowMinutes),
                            QueueProcessingOrder = QueueProcessingOrder.OldestFirst,
                            QueueLimit           = config.PerUser.QueueLimit
                        });
                });
            });
    }

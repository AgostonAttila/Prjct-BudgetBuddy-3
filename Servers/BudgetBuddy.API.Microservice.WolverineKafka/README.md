...
[![Wolverine](https://img.shields.io/badge/Wolverine-3.x-6B21A8)](https://wolverine.netlify.app/)
[![Kafka](https://img.shields.io/badge/Kafka-transport-231F20?logo=apachekafka)](https://kafka.apache.org/)
...
Personal finance management REST API — the microservices edition of BudgetBuddy. Each domain is a fully independent service with its own database, Wolverine message bus, and deployment unit. An API Gateway (YARP) is the single entry point for all clients.

## Architecture

This project is a **microservices platform**: each domain ships as a separately deployable process. Services communicate asynchronously through **Wolverine** (with Kafka as the transport backend) and synchronously through the **API Gateway** (YARP). There are no direct service-to-service HTTP calls in the critical path.

Each service:

- owns its own **PostgreSQL database** (no shared DB, no cross-schema queries)
- publishes and consumes domain events via **Wolverine message handlers** over Kafka (`WolverineFx.Kafka`)
...
- maintains local **read models** (snapshots) populated by Wolverine message handlers — no cross-service queries at runtime
...
| `Analytics` | `analytics` | Dashboard, spending trends, investment performance reports (read models from domain events) |
...
**Messaging:**
- **Wolverine** (JasperFx) — message bus abstraction; replaces direct Confluent.Kafka usage
  - Transport: `WolverineFx.Kafka` — Kafka brokers as the underlying transport
  - Outbox: `WolverineFx.EntityFrameworkCore` — `WithEfCoreOutbox()` per service
  - Envelope storage: `MapWolverineEnvelopeStorage("wolverine")` per service schema
  - Dead-letter queues: per-domain Kafka DLQ topics via Wolverine error handling
...
- Event-driven architecture with Wolverine over Kafka
- Transactional Outbox built into Wolverine (EF Core integration, no separate outbox processor)
- Saga pattern for distributed transactions (Wolverine stateful sagas)
...
Services communicate exclusively through Wolverine domain events over Kafka. Each service maintains local snapshots (read models) populated by its Wolverine message handlers — so query-side operations never need to call another service.
...
Each domain has a Dead Letter Queue (`budgetbuddy.<domain>.dlq`, 7-day retention) for failed messages — managed by Wolverine's error handling pipeline.
...
Wolverine provides a built-in transactional outbox via `WithEfCoreOutbox()`. Domain events are persisted within the same DB transaction as the domain write, then delivered to Kafka by Wolverine's internal relay. No separate outbox processor job is needed — Wolverine owns the delivery lifecycle.
...
|-----|---------|----------------|-------------|
| `DailyPriceSnapshotJob` | Investments | 22:00 | Fetches closing prices for all tracked symbols |
| `DailyFxSnapshotJob` | Investments | 16:00 | Fetches ECB exchange rates |
| `BackfillMarketDataJob` | Investments | 03:00 | Detects and fills historical price/FX gaps |
...
│   │   ├── Consumers/                          # Wolverine handlers: transaction events → account snapshot
│   │   ├── Persistence/
...
│   │   ├── Consumers/                          # Wolverine handlers: account/category snapshots, saga events
...
│   │   ├── Consumers/                          # Wolverine handlers: transaction events → spending tracking
│   │   ├── Persistence/
│   │   │   ├── BudgetsDbContext.cs
│   │   │   └── Migrations/
│   │   └── Program.cs
...
│   │   ├── Consumers/                          # Wolverine handlers: account and price updates
│   │   ├── Persistence/
│   │   │   ├── InvestmentsDbContext.cs
│   │   │   └── Migrations/
│   │   └── Program.cs
│   │
│   ├── BudgetBuddy.Service.Analytics/          # DB: analytics (read models from domain events)
│   │   ├── Features/
...
│   │   ├── Consumers/                          # Wolverine handlers: all domain events → local projections
│   │   └── Persistence/
...
│       ├── Consumers/                          # Wolverine handlers: budget alerts, user events
│       └── Program.cs
│
├── Shared/
...
│   │   ├── Persistence/
│   │   │   └── Interceptors/                   # RLS, AuditLog interceptors
│   │   ├── Messaging/                          # Wolverine + Kafka setup, DLQ handling
...
│   │   ├── Jobs/                               # Audit retention, saga timeout (Quartz base)
│   │   └── Logging/                            # Serilog configuration, Seq integration
│   │
│   └── BudgetBuddy.Shared.Messages/            # Wolverine/Kafka event contracts
│       ├── Topics/                             # Kafka topic name constants
│       ├── Consumer/                           # Consumer configuration
│       ├── Events/                             # Domain event definitions per service
...
│   └── BudgetBuddy.Integration.Tests/         # Integration tests (Testcontainers: Postgres + WireMock)
│
└── Infrastructure/
...
All topics follow the convention `budgetbuddy.<domain>.<event>`. Wolverine handles subscription and publishing via `WolverineFx.Kafka` — topics are the Kafka-level transport detail.
...
cd Servers/BudgetBuddy.API.Microservice.Wolverine
...
| `Kafka__BootstrapServers` | Yes | Kafka broker address (Wolverine transport) |
...
**Startup sequence (for services with a database):**
1. DbContext auto-migrated on startup
2. Wolverine envelope storage schema applied (`wolverine` schema in the service's PostgreSQL database)
...
5. Wolverine message bus starts and begins processing inbound Kafka messages

> Financial and Notifications services have no database — they skip steps 1–2.
...
# Analytics
dotnet ef migrations add <MigrationName> \
  --project Services/BudgetBuddy.Service.Analytics \
  --startup-project Services/BudgetBuddy.Service.Analytics \
...
**Integration tests** use Testcontainers to spin up real PostgreSQL instances, WireMock.Net for external HTTP providers (Yahoo Finance, CoinGecko, Frankfurter), and Respawn for database cleanup between tests.
...
**Wolverine messages not being processed:**
- Check Seq for handler exceptions or deserialization errors
- Inspect `wolverine.incoming_envelopes` in the service's PostgreSQL database for stuck messages
- Dead-lettered messages land in `budgetbuddy.<domain>.dlq` Kafka topic (visible in Kafka UI at `http://localhost:8090`)
...
- ✅ Kafka cluster (minimum 3 brokers for production, KRaft)
- ✅ Wolverine envelope storage schema applied (automatic on startup)
...
3. Verify Wolverine message processing is active (check `wolverine.incoming_envelopes` is draining)
4. Verify Quartz jobs appear in logs within their first scheduled window
5. Set up monitoring alerts on `/health/ready` for on-call
6. Implement data retention policy for DLQ topics (7-day default; archive or purge)
7. Schedule regular PostgreSQL backups per service database
8. Configure Keycloak brute force protection and session limits in realm settings

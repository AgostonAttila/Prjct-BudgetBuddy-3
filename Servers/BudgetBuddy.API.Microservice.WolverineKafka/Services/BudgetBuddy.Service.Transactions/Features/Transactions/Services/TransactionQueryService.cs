...
namespace BudgetBuddy.Service.Transactions.Features.Transactions.Services;
    ...
    public Task<List<CategorySpendingRow>> GetCategorySpendingAsync(
        ...
        context.Transactions
            ...
            .Where(t => t.UserId == userId &&
                        ...
                        t.CategoryId.HasValue &&
                        t.CurrencyCode != null)
            // When RefCurrencyAmount is set (captured at transaction time in EUR), group those
            // rows under "EUR" and sum the pre-converted amount — this avoids live-rate drift
            // when comparing past spending against a budget (#7).
            // Older rows without RefCurrencyAmount keep their original currency and fall back
            // to live FX conversion in BudgetAlertService (no worse than before).
            .GroupBy(t => new
            {
                t.CategoryId,
                Currency = t.RefCurrencyAmount.HasValue ? "EUR" : t.CurrencyCode
            })
            .Select(g => new CategorySpendingRow(
                g.Key.CategoryId!.Value,
                g.Key.Currency!.ToUpper(),
                g.Sum(t => t.RefCurrencyAmount.HasValue ? t.RefCurrencyAmount.Value : t.Amount)))
            .ToListAsync(cancellationToken);

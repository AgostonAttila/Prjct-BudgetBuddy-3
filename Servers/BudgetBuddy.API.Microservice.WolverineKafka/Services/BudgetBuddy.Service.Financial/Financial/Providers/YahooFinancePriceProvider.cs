...
namespace BudgetBuddy.Service.Financial.Financial.Providers;
    ...
    ILogger<YahooFinancePriceProvider> logger) : IPriceProvider, IHistoricalPriceProvider
{
    ...
    private HttpClient CreateClient() => httpClientFactory.CreateClient(FinancialHttpClientExtensions.YahooClientName);

#pragma warning disable S1075 // URIs should not be hardcoded — Yahoo Finance API endpoints have no configuration alternative
    private const string YahooCookieUrl = "https://fc.yahoo.com";
    private const string YahooCrumbUrl  = "https://query1.finance.yahoo.com/v1/test/getcrumb";
#pragma warning restore S1075
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        logger.LogDebug("Yahoo Finance: fetching prices for {Symbols} in {Currency}", symbolsParam, currency);

        var response = await CreateClient().GetFromJsonAsync<YahooQuoteResponse>(url, cancellationToken);

        var quotes = response?.QuoteResponse?.Result;
        if (quotes is null || quotes.Count == 0)
        {
            logger.LogWarning("Yahoo Finance: empty response for {Symbols}, refreshing crumb", symbolsParam);
            await cache.RemoveAsync(CrumbCacheKey, cancellationToken);

            var freshCrumb = await GetCrumbAsync(cancellationToken);
            var retryUrl = $"v7/finance/quote?symbols={symbolsParam}&crumb={Uri.EscapeDataString(freshCrumb)}&lang=en-US&region=US";
            response = await CreateClient().GetFromJsonAsync<YahooQuoteResponse>(retryUrl, cancellationToken);
            quotes = response?.QuoteResponse?.Result;

            if (quotes is null || quotes.Count == 0)
            {
                logger.LogWarning("Yahoo Finance: still empty after crumb refresh for {Symbols}", symbolsParam);
                return [];
            }
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var response = await CreateClient().GetFromJsonAsync<YahooChartResponse>(url, cancellationToken);
    ...
    private async Task<string> GetCrumbAsync(CancellationToken cancellationToken)
    {
        return await cache.GetOrCreateAsync(
            ...
            async ct =>
            {
                ...
                await client.GetAsync(YahooCookieUrl, ct);
                var crumb = await client.GetStringAsync(YahooCrumbUrl, ct);

    ...
    : SaveChangesInterceptor
{
    ...
    {
        // Only AppDbContext has AuditLog in its model — skip all others
        if (context.Model.FindEntityType(typeof(AuditLog)) == null)
        {
            return;
        }

        // Skip audit logging for bulk operations (e.g. BulkInsertAsync via IBulkOperationContext)
        if (context is IBulkOperationContext { BulkOperationInProgress: true })
        {
            return;
        }

        // Resolve auditable entries first — skip all expensive work if nothing to audit
        ...
        var userIdentifier = httpContext?.User.FindFirst("email")?.Value

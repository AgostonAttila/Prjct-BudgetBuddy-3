...
public static class CachingExtensions
{
    public static void AddCaching(this IServiceCollection services,IConfiguration configuration, IHostEnvironment environment)
    {
        ...
        services.AddHybridCache(options =>
        {
            ...
            // L2 (Redis): 5 min — shared across all pods.
            // L1 (in-memory): 3 min jittered (170–190 s) — meaningfully shorter than L2 so stale
            // data is bounded, but long enough to absorb traffic bursts within a pod without
            // hitting Redis on every request. Jitter staggers per-instance expiry to prevent
            // a thundering herd where all pods miss L1 simultaneously and pile onto Redis (P-03).
            options.DefaultEntryOptions = new HybridCacheEntryOptions
            {
                Expiration = TimeSpan.FromMinutes(5),
                LocalCacheExpiration = TimeSpan.FromSeconds(170 + Random.Shared.Next(0, 20))
            };
        });
        ...
        services.AddOutputCache(options =>
        {
            ...
            // ============================================================
            // Item 16: Explicit policies for remaining endpoint types
            // ============================================================

            // Live market price data — very short TTL, no user variation needed
            // Prices change every few seconds; 30 s is a safe stale-while-usable window.
            options.AddPolicy("market-prices", builder =>
                builder.Expire(TimeSpan.FromSeconds(30))
                    .Tag("market-prices")
                    .SetVaryByQuery("symbol", "currency", "type"));

            // Notifications / alerts — personal, short TTL
            ...
            // Mutation endpoints (POST/PUT/DELETE) must never be cached.
            // Apply [OutputCache(NoStore = true)] directly on write endpoint handlers,
            // or omit [OutputCache] altogether — Output Cache is opt-in per endpoint.
        });
    }
}
...
namespace BudgetBuddy.Shared.Infrastructure.Filters;
    ...
    ILogger<IdempotencyFilter> logger) : IEndpointFilter
{
        ...
        EndpointFilterDelegate next)
    {
        ...
        {
            logger.LogWarning(
                "Request to {Path} rejected: missing Idempotency-Key header.",
                httpContext.Request.Path);

            return Results.BadRequest(new
            {
                error = "Missing Idempotency-Key",
                message = "This endpoint requires an Idempotency-Key header to prevent duplicate requests."
            });
        }
